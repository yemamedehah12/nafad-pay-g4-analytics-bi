...
INSERT INTO fact_transactions (
    ...
    channel, device_type,
    sync_status, is_conflict, is_unresolvable_conflict,
    is_lagging, is_pending_sync, is_cross_dc, has_clock_skew,
...
SELECT
    ...
    -- CONFLICT non résolvable : sync_status=CONFLICT et aucun montant nodal disponible
    (
        t.sync_status = 'CONFLICT'
        AND (TRIM(COALESCE(t.amount_node_a, '')) = '')
        AND (TRIM(COALESCE(t.amount_node_b, '')) = '')
    )                                                   AS is_unresolvable_conflict,
    ...
    -- Clock skew : completed_at antérieur à created_at
    (
        TRIM(COALESCE(t.completed_at, '')) != ''
        AND TRIM(COALESCE(t.created_at, '')) != ''
        AND TRIM(t.completed_at)::TIMESTAMP < TRIM(t.created_at)::TIMESTAMP
    )                                                   AS has_clock_skew,
